...
# ==========================
# Cache server-side por assinatura (coleta)
# ==========================
...
# ==========================
# Sidebar (layout referência) — tudo em um único form
# ==========================
def _sidebar_form(pncp_df: pd.DataFrame, ibge_df: Optional[pd.DataFrame]):
    ...
    with st.sidebar.form(key="filtros_form"):
        ...
        # Municípios (IBGE-like → PNCP), botão SEMPRE visível dentro do form
        ...
        # Lista dos selecionados (apenas exibição)
    ...
    # AÇÕES fora do form (persistência)
    if excluir:
...
def main():
    ...
    # Sidebar em formulário (referência)
